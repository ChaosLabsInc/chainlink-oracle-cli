...
contract AggregatorMultipliedStep is AggregatorV3Interface {
    ...
    uint internal s_stepBlocks;
    // block - the block at the last time the values were changed, where we start the count from.
    uint internal s_block;

    constructor(uint80  roundId, int256  answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound, int256 stepChange, uint stepBlocks) public {
        s_roundId= roundId;
        s_answer= answer;
    ...
    function setValues(uint80  roundId, int256  answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound, int256 stepChange, uint stepBlocks) public {
        s_roundId= roundId;
        s_answer= answer;
        s_startedAt = startedAt;
        s_updatedAt= updatedAt;
    ...
    */
    ...
    function getRoundData(uint80 _roundId) view public override
        returns (
           ...
           int256 mocked_answer = s_answer * s_stepChange *  ( 1 + int256( ((block.number - s_block) / s_stepBlocks) ));
